...
#define MAX_CHARS_PER_LINE 16 // Adjust based on your display and font size
#define MAX_LINES 4
String lines[MAX_LINES]; // Array to hold lines of text
int currentLine = 0; // Index of the current line being used

void addText(String text) {
    // Check if current line + new text exceeds max characters per line
    if (lines[currentLine].length() + text.length() > MAX_CHARS_PER_LINE) {
    ...
    for (int i = 0; i <= currentLine; i++) {
        display.setCursor(0, i * 8); // Move down 8 pixels per line
...
class MyCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) override {
        ...
        if (!value.empty()) {
            ...
            addText(text); // Use the addText function to handle display updates
